...
export function AssetRow({ asset, rank, watched, onToggleWatch }: AssetRowProps) {
  return (
    <Link
      ...
      className="grid grid-cols-[1.5rem_2.25rem_minmax(0,1fr)_8rem_2rem] items-center gap-3 px-4 py-3 table-row last:border-0"
    >
      <span className="text-xs text-text-muted text-center">{rank ?? ''}</span>
      ...
      <span className="flex justify-center">
        {onToggleWatch !== undefined && (
          <button
            ...
            className="p-1.5 hover:bg-surface-2 transition-colors"
